...
public:
    ...
    void setExDays(int i){exposedDays=i;}
    void setDDAfter(int i){dayDiesAfter = i;}
    void setResistance(double i){Resistance = i;}
    ...
    void display(){
        cout<<"person("<<PID<<", "<<Age<<", "<<Resistance<<", "<<Fid
            <<", "<<symptom<<", "<<exposedDays<<", "
...
private:
    ...
    int Age;
    double Resistance; 
    string symptom;   ///notex, infected or fever or pustules etc..
    int exposedDays;
    int incubateDays;  ///the number of days until fever begins
    string isDiagnosed;
    int dayDiesAfter;   ///the number of days after fever when 30% die
    list<int>mixgrps;
    list<int>visited;
